...
function generate(e) {
  ...
  qr_code_element.appendChild(t);
  let r = document.createElement("a");
  r.setAttribute("download", "qr_code.png"),
    (r.innerHTML = 'Download <i class="fa-solid fa-download"></i>'),
    t.appendChild(r);
  let n = document.querySelector(".qr-code img"),
    o = document.querySelector("canvas");
  null == n.getAttribute("src")
    ? setTimeout(() => {
        r.setAttribute("href", `${o.toDataURL()}`);
      }, 300)
    : setTimeout(() => {
        r.setAttribute("href", `${n.getAttribute("src")}`);
      }, 300);
}
var cameraId;
btn.addEventListener("click", () => {
  let e = document.querySelector("#input_text");
  "" != e.value
    ? (0 == qr_code_element.childElementCount ||
        (qr_code_element.innerHTML = ""),
      generate(e))
    : ((document.querySelector(".error").style = ""),
      (document.querySelector(".error").innerHTML = "Invalid Input!"));
}),
  generate({ value: "https://qr-codes.vercel.app" }),
  document.querySelectorAll(".select-section button").forEach((e) => {
  ...
  Html5Qrcode.getCameras()
    .then((e) => {
      if (
        ...
        let t = document.querySelector("#camera");
        for (let r = 0; r < e.length; r++) {
          let n = document.createElement("option");
          (n.value = e[r].id), (n.innerHTML = e[r].label), t.appendChild(n);
        }
        document.querySelector(".scan").addEventListener("click", () => {
          "" !== (cameraId = t.options[t.selectedIndex].value)
            ? ((document.querySelector("#camera").style = "display: none;"),
              scanner
                .start(
                  cameraId,
                  { fps: 24, qrbox: { width: 300, height: 300 } },
                  (e, t) => {
                    (document.querySelector(".res").innerText = `${e}`),
                      (document.querySelector(".res").style = ""),
                      (document.querySelector(".copy-btn").style = ""),
                      console.log(`Code matched = ${t}`),
                      document
                        .querySelector(".res")
                ...
                .then(() => {
                  console.log("camera started"),
                    (document.querySelector(".scan").disabled = !0),
                    (document.querySelector(".stop-btn").disabled = !1),
    ...
    .catch((e) => {
      console.error(e),
        ...
        (document.querySelector(".scan-btn").disabled = !0),
  ...
  scanner
    ...
    .then((e) => {
      console.log("Scan stopped"),
        ...
        (document.querySelector(".scan").disabled = !1),
        (document.querySelector(".stop-btn").disabled = !0),
    ...
    navigator.clipboard
      ? navigator.clipboard
          ...
          .catch((t) => {
            console.error(t),
              (e.target.innerHTML =
                'Copy failed <i class="fa-solid fa-xmark"></i>'),
              setTimeout(() => {
                e.target.innerHTML = "Copy";
              }, 1500);
          })
